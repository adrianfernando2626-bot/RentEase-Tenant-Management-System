...
$sql1 = "SELECT * FROM contract WHERE contract_id = " . $id;
$rs1 = mysqli_query($db_connection, $sql1);
$user_ids = mysqli_fetch_array($rs1);
$user_id = $user_ids['user_id'];
$sql = "SELECT user_id, first_name, last_name, email, room_id FROM userall WHERE user_id = " . $user_id;
...
if ($id) {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Contract Termination Notice';
    ...
    header("Location: ../contractmanage.php?message=contract_deleted");
    exit();
} else {
